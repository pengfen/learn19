...
func TestParseCityList(t *testing.T) {
	...
	if len(result.Requests) != resultSize { // 对比url总记录数
		t.Errorf("result should have %d  requests: but had %d", resultSize, len(result.Requests))
	}

	for i, url := range expectedUrls {
		if result.Requests[i].Url != url { // 对比url前三条记录
			t.Errorf("expected url #%d: %s; but was %s", i, url, result.Requests[i].Url)
		}
	}

	if len(result.Items) != resultSize { // 对比item总记录数
		t.Errorf("result should have %d requests: but had %d", resultSize, len(result.Items))
	}
	for i, city := range expectedCities {
		if result.Items[i].(string) != city { // 对比item前三条记录
